...
export default function Analysis() {
  ...
  const totalAssetsValue = assets.reduce((acc, asset) => acc + Number(asset.current_value || asset.value), 0);
      ...
      <main className="flex-1 px-6 pt-16 pb-48">
        <section className="mb-12 relative group">
          ...
          <div className="neon-border rounded-[2.5rem] p-4 bg-transparent relative flex flex-col items-center justify-center min-h-[240px] w-full overflow-hidden">
            <label className="absolute top-8 text-[10px] uppercase tracking-[0.5em] text-zinc-500 font-semibold">PATRIMÔNIO TOTAL</label>
            <div className="flex flex-col items-center justify-center w-full px-2">
              <span className="text-white text-[11vw] xs:text-5xl font-display font-light tracking-tighter premium-text-glow leading-none text-center whitespace-nowrap">
        ...
        <section className="mb-14 flex items-center justify-between">
          <div className="relative w-44 h-44 animate-pulse-neon">
            ...
            <svg className="w-full h-full relative z-10" viewBox="0 0 100 100">
              ...
              <circle className="concentric-ring animate-draw ring-1" cx="50" cy="50" r="45" stroke="#0FB67F" strokeWidth="2.5"></circle>
              <circle className="concentric-ring animate-draw ring-2" cx="50" cy="50" opacity="0.7" r="38" stroke="#0FB67F" strokeWidth="2.5"></circle>
              <circle className="concentric-ring animate-draw ring-3" cx="50" cy="50" opacity="0.4" r="31" stroke="#0FB67F" strokeWidth="2.5"></circle>
            </svg>
          ...
          <div className="flex-1 ml-10 space-y-5">
            <h3 className="text-[10px] uppercase tracking-[0.4em] text-zinc-500 font-bold">DISTRIBUIÇÃO</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between text-[12px] tracking-widest">
                <span className="text-zinc-500">RENDA FIXA</span>
                <span className="font-bold text-white">{distFixed}%</span>
              </div>
              <div className="flex items-center justify-between text-[12px] tracking-widest">
                <span className="text-zinc-500">RENDA VAR.</span>
                <span className="font-bold text-white">{distVar}%</span>
              </div>
              <div className="flex items-center justify-between text-[12px] tracking-widest">
                <span className="text-zinc-500">CRIPTO</span>
        ...
        <section>
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-[10px] font-display font-bold tracking-[0.3em] text-white uppercase">MEUS ATIVOS</h2>
          ...
          <div className="space-y-4">
            {assets.length > 0 ? (
                ...
                >
                  <div className="flex items-center space-x-5">
                    <div className="w-11 h-11 border border-zinc-800 rounded-full flex items-center justify-center">
                      <span className="material-symbols-outlined text-white text-xl">{asset.icon || 'currency_bitcoin'}</span>
                    </div>
                    <div>
                      <p className="text-sm font-bold font-display tracking-widest text-white">{asset.ticker || 'ATIVO'}</p>
                      <p className="text-[9px] text-zinc-500 font-medium uppercase tracking-widest mt-0.5">{asset.name}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold text-white">R$ {Number(asset.current_value || asset.value).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
              ...
              <div className="py-12 text-center neon-border rounded-3xl p-8 bg-black/20">
                <p className="text-[9px] text-zinc-500 uppercase tracking-widest mb-4">Portfólio vazio</p>
                <button
